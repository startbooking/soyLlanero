...
const PaymentPage = () => {
  ...
  const handleWompiPaymentOld2 = async () => {
    setIsProcessing(true);
  ...
  const handleWompiPayment = async () => {
    ...
    checkout.open((res: any) => {
      ...
      // Objeto con toda la información que queremos pasar a la siguiente pantalla
      ...
      if (transaction.status === 'APPROVED') {
        ...
        // Redirección a Error con el detalle de Wompi
        navigate("/payment-error", {
          state: {
            reservation: reservationData,
            errorDetail: transaction.status_message || "La transacción no pudo ser procesada por el banco."
          },
          replace: true
        });
      }
